      ...
      <g fill="none" stroke={`url(#mg-${inner ? "i" : "o"})`} strokeWidth="0.9" strokeLinecap="round">
        ...
        {Array.from({ length: petals }).map((_, i) => (
          <g key={i} transform={`rotate(${(i * 360) / petals} 300 300)`}>
            <path
              d={`M300 ${inner ? 110 : 30} C 320 ${inner ? 170 : 110}, 320 ${inner ? 220 : 200}, 300 ${inner ? 250 : 270} C 280 ${inner ? 220 : 200}, 280 ${inner ? 170 : 110}, 300 ${inner ? 110 : 30} Z`}
            />
        ...
        {Array.from({ length: petals / 2 }).map((_, i) => (
          <g key={`p-${i}`} transform={`rotate(${(i * 360) / (petals / 2) + 11.25} 300 300)`}>
            <path d="M300 300 C 330 330, 360 360, 380 410 C 340 380, 310 350, 300 300 Z" opacity="0.6" />
          </g>
        ))}
...
function DiyaFlicker({ size = 28 }: { size?: number }) {
  return (
    <svg viewBox="0 0 48 48" width={size} height={size} aria-hidden="true">
      <defs>
        <radialGradient id={`flameAmber-${size}`} cx="0.5" cy="0.7" r="0.6">
      ...
      <g style={{ transformOrigin: "24px 22px", animation: "heroFlame 1.5s ease-in-out infinite" }}>
        <ellipse cx="24" cy="20" rx="3.4" ry="7" fill={`url(#flameAmber-${size})`} />
        ...
        >
          ...
          {p.type === "petal" && (
            <svg viewBox="0 0 10 14" width={p.size * 1.6} height={p.size * 2}>
              <path d="M5 0 C 9 4, 9 10, 5 14 C 1 10, 1 4, 5 0 Z" fill="#E8C766" />
            </svg>
          )}
        </span>
      ))}
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ inset: 7, border: "1.5px solid rgba(201,168,76,0.5)" }}
      />
      <div
